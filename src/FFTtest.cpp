...
typedef std::complex<float> InputType[N];
typedef std::complex<float> OutputType[N];
...
int main(int argc, char *argv[])
{
  ...
  try {
    ...
    //hostInput[3] = 3;
    for (int i = 0; i < N; i ++)
      hostInput[i] = std::complex<float>(2 * i - 5, 3 * i - 2);

    //queue.enqueueUnmapMemObject(deviceBuffer, ptr);

    cl::Kernel       kernel(program, "fft32test");
